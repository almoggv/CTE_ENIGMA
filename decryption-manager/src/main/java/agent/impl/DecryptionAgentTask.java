...
public class DecryptionAgentTask extends DecryptionAgent {
    ...
    static {
        try {
            ...
            p.load(DecryptionAgentTask.class.getResourceAsStream(PropertiesService.getLog4jPropertiesResourcePath()));
            PropertyConfigurator.configure(p);      //Dont forget here
            log.debug("Logger Instantiated for : " + DecryptionAgentTask.class.getSimpleName());
        } catch (IOException e) {
            System.out.println("Failed to configure logger of -" + DecryptionAgentTask.class.getSimpleName() ) ;
        }
    }

    public DecryptionAgentTask(EncryptionMachine encryptionMachine) {
...
//        MachineState init = machineInitialState.getDeepClone();
        ...
        if(checkIfInDictionary(decryptionResult)) {
            System.out.println("FOUND CANDIDATE = " + decryptionResult +" by: " + this.id);
    ...
    protected Boolean call() throws Exception {
        System.out.println("Agent with id=" + this.id + " was called");
